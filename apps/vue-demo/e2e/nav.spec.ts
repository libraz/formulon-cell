...
test('E03 (vue-demo): Tab / Shift+Tab / Arrow navigate the grid', async ({ page }) => {
  await runArrowAndTabNavScenario(page);
});

test('E04 (vue-demo): F2 → ESC discards the in-progress edit', async ({ page }) => {
  await runF2EscapeScenario(page);
});

test('Arrow sequence (vue-demo): Arrow keys + Enter walk the grid', async ({ page }) => {
