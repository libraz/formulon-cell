...
test('Fill-down (react-demo): Mod+D propagates the anchor value down the selection', async ({
  page,
}) => {
  await runFillDownScenario(page);
});

// Phase 2: ribbon Fill > Down used to silent-fail in React because
...
test('Ribbon Fill (react-demo): clicking the Fill > Down menu fills the selected range', async ({
  page,
}) => {
