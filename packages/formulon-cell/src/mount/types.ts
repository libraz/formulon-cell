...
export interface MountOptions {
   ...
   *  theme together. Access it afterwards via `instance.toolbar`.
   *
   *  Pass `true` for the sensible defaults (locale-derived language, dynamic
   *  dropdowns). Pass a `MountToolbarOptions` object to add backstage content,
   *  custom hooks, submenu factories, a ribbon-tab profile, or lifecycle
   *  callbacks while keeping the single-call mount — the options you set are
   *  merged over (and override) those defaults. The toolbar is wired against
   *  the freshly mounted instance, so `instance`/host wiring stays automatic. */
  toolbar?: boolean | MountToolbarOptions;
