...
export function formatCodeToNumFmt(code: string): NumFmt | null {
  ...
  const normalized = normalizeExcelFormatCode(code);
  if (!normalized || normalized === 'General') return null;
  code = normalized;
  const probe = firstExcelFormatSection(code);
  ...
  // Accounting formats generated by Excel commonly use `_` spacing and `*`
...
function firstExcelFormatSection(code: string): string {
  let out = '';
  let inQuote = false;
  let inBracket = false;
...
function normalizeExcelFormatCode(code: string): string {
  return code
    .trim()
    .replace(/\[\$([^\]-]+)(?:-[^\]]+)?\]/g, '$1')
    .replace(/\[\$-[^\]]+\]/g, '')
